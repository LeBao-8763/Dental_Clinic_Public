...
const ScheduleArrange = () => {
  ...
  const clinicSchedule = [
    ...
    const dayData = clinicHoursData.find((item) => item.day_of_week === dayEnum);
    return {
  ...
  const toggleDay = (dayId) => {
    if (expandedDay === dayId) {
      ...
      if (typeof dayId === "number") {
        ...
        const mergedSlots = [...new Set([...currentSlots, ...existingSlotIndices])];
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-50 min-h-screen">
      ...
      <p className="text-gray-600 mb-6">Hãy chọn những lịch khám phù hợp để đem tới cho bệnh nhân chất lượng dịch vụ tốt nhất.</p>

      <div className="bg-teal-50 border border-teal-200 rounded-lg p-4 mb-6">
        <div className="flex items-start justify-between">
          <div className="flex items-start flex-1">
            <span className="text-teal-600 mr-2">ⓘ</span>
            <span className="text-teal-800">Phòng khám hoạt động mỗi ngày từ 7:00-17:00</span>
          </div>
          <button onClick={() => setShowScheduleDetail(true)} className="text-sm hover:underline ml-4 whitespace-nowrap" style={{ color: "#009688" }}>Xem chi tiết</button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full shadow-xl">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              ...
              <button onClick={() => setShowScheduleDetail(false)} className="text-gray-400 hover:text-gray-600 text-2xl leading-none">×</button>
            </div>
            <div className="p-6">
              <table className="w-full">
                <thead>
                  <tr className="border-b-2 border-gray-300">
                    <th className="text-left py-3 px-2 font-semibold text-gray-700">Ngày</th>
                    <th className="text-right py-3 px-2 font-semibold text-gray-700">Giờ hoạt động</th>
                  </tr>
                </thead>
                <tbody>
                  {clinicSchedule.map((item, idx) => (
                    <tr key={idx} className="border-b border-gray-200 last:border-0">
                      <td className="py-3 px-2 font-medium text-gray-700">{item.day}</td>
                      <td className={`py-3 px-2 text-right ${item.hours === "Nghỉ" ? "text-red-500 font-medium" : "text-gray-600"}`}>{item.hours}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="p-6 pt-0">
              <button onClick={() => setShowScheduleDetail(false)} className="w-full text-white font-medium py-3 rounded-lg transition-opacity hover:opacity-90" style={{ backgroundColor: "#009688" }}>Đóng</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="mb-6 border-b border-gray-300">
        <div className="flex">
          <button onClick={() => setActiveTab("weekly")} className={`px-6 py-3 border-b-2 font-medium ${activeTab === "weekly" ? "" : "border-transparent text-gray-600"}`} style={activeTab === "weekly" ? { borderColor: "#009688", color: "#009688" } : {}}>
            Lịch làm việc (Thứ 2-7)
          </button>
          <button onClick={() => setActiveTab("specific")} className={`px-6 py-3 border-b-2 font-medium ${activeTab === "specific" ? "" : "border-transparent text-gray-600"}`} style={activeTab === "specific" ? { borderColor: "#009688", color: "#009688" } : {}}>
            Chọn ngày cụ thể
          </button>
        </div>
      </div>
      ...
      {activeTab === "weekly" ? (
        <div className="space-y-4">
          {days.map((day) => (
            <div key={day.id} className="bg-white rounded-lg border border-gray-300 overflow-hidden">
              <div className="p-4 flex items-center justify-between">
                <div>
                  <h3 className="font-bold text-lg">{day.name}</h3>
                  {day.notSelected && <p className="text-gray-500 text-sm">Chưa chọn thời gian</p>}
                  {day.hasSlots && !day.notSelected && (
                    <div className="flex gap-2 mt-2 flex-wrap">
                      {day.slots.map((slot, idx) => (
                        <span key={idx} className="px-3 py-1 rounded text-sm" style={{ backgroundColor: "#B2DFDB", color: "#00695C" }}>{slot}</span>
                      ))}
                      {day.totalSlots > 5 && (
                        <span className="px-3 py-1 rounded text-sm" style={{ backgroundColor: "#B2DFDB", color: "#00695C" }}>+{day.totalSlots - 5}</span>
                      )}
                    </div>
                  )}
                </div>
                <button onClick={() => toggleDay(day.id)} className="flex items-center gap-2 hover:opacity-80" style={{ color: "#009688" }}>
                  <Edit size={18} />
                  <span>Chỉnh sửa</span>
                </button>
              </div>
              ...
              {expandedDay === day.id && (
                <div className="border-t border-gray-200 p-4 bg-gray-50">
                  <p className="text-sm mb-3"><span className="font-medium">Đã chọn: </span><span style={{ color: "#009688" }}>{(tempSelectedSlots[day.id] || []).length}</span></p>

                  <div className="grid grid-cols-6 gap-2 mb-4">
                    {timeSlots.map((slot, idx) => {
                      const isSelected = (tempSelectedSlots[day.id] || []).includes(idx);
                      const dayEnum = dayEnums[day.id - 2];
                      const existingSlotIndices = getSlotIndicesForSchedule(dayEnum);
                      const isFromSchedule = existingSlotIndices.includes(idx);

                      return (
                        <button key={idx} onClick={() => toggleSlot(day.id, idx)} className={`py-2 px-3 rounded text-sm border transition-colors ${isSelected ? "text-white" : "bg-white text-gray-700 border-gray-300"} ${isFromSchedule && !isSelected ? "ring-2 ring-teal-400" : ""}`} style={isSelected ? { backgroundColor: "#009688", borderColor: "#00796B" } : {}}>
                          {slot}
                        </button>
                      );
                    })}
                  ...
                  <button onClick={() => handleConfirm(day.id)} className="w-full text-white font-medium py-3 rounded-lg transition-opacity hover:opacity-90" style={{ backgroundColor: "#009688" }}>
                    Xác nhận
                  </button>
                </div>
              )}
          ...
          <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-white p-4 rounded-lg border border-gray-200">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <button onClick={prevMonth} className="p-2 rounded hover:bg-gray-100"><ChevronLeft size={18} /></button>
                    <div className="font-semibold">
                      {calendarMonth.toLocaleString("vi-VN", { month: "long" })} {calendarMonth.getFullYear()}
                    </div>
                    <button onClick={nextMonth} className="p-2 rounded hover:bg-gray-100"><ChevronRight size={18} /></button>
                  </div>
                  <div className="text-sm text-gray-500">Chọn ngày để chỉnh lịch / xin nghỉ</div>
                </div>

                <div className="grid grid-cols-7 gap-1 text-center">
                  {daysOfWeekShort.map((d, i) => (
                    <div key={i} className="text-xs font-medium text-gray-600 py-2">{d}</div>
                  ))}
                  ...
                  {calendarGrid.map((d, idx) => {
                    ...
                    const disabled = iso < minDateString || !isClinicOpenOnDate(d);

                    return (
                      <button
                        key={idx}
                        onClick={() => handlePickDateFromCalendar(d)}
                        className={`p-2 h-12 flex items-center justify-center rounded transition-all text-sm ${isCurrentMonth ? "" : "opacity-40"} ${disabled ? "cursor-not-allowed" : "hover:bg-gray-100"}`}
                        disabled={disabled}
                        title={isClinicOpenOnDate(d) ? "Phòng khám mở" : "Phòng khám đóng"}
                      >
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center ${isSelected ? "bg-[#009688] text-white" : isToday ? "ring-1 ring-[#009688]/40" : ""}`}>
                          <span className={`${disabled ? "line-through text-gray-400" : ""}`}>{d.getDate()}</span>
                        </div>
                      </button>
                    );
                  })}
                ...
                <div className="mt-3 text-xs flex items-center gap-3">
                  <div className="flex items-center gap-2"><div className="w-3 h-3 bg-[#009688] rounded" /> Đã chọn</div>
                  <div className="flex items-center gap-2"><div className="w-3 h-3 bg-gray-300 rounded" /> Ngày khác tháng</div>
                  <div className="flex items-center gap-2"><div className="w-3 h-3 bg-red-200 rounded" /> Phòng khám nghỉ</div>
                </div>
            ...
            <div className="md:col-span-1">
              <div className="bg-white p-4 rounded-lg border border-gray-200 h-full">
                ...
                <p className="text-sm text-gray-600 mb-2">Ngày chọn: <span className="font-medium text-gray-800">{selectedDate ? formatDate(selectedDate) : "—"}</span></p>
                <p className="text-sm text-gray-600 mb-4">Lịch phòng khám (theo tuần):</p>
                <div className="text-sm">
                  {clinicSchedule.map((c, i) => (
                    <div key={i} className="flex items-center justify-between py-1 text-xs">
                      <span className="text-gray-700">{c.day}</span>
                      <span className={`text-right ${c.hours === "Nghỉ" ? "text-red-500" : "text-gray-600"}`}>{c.hours}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-4">
                  <button onClick={() => {
                    ...
                    const found = calendarGrid.find(d => getISODate(d) >= minDateString && isClinicOpenOnDate(d));
                    if (found) handlePickDateFromCalendar(found);
                    else toast.info("Không tìm thấy ngày khả dụng trong tháng này");
                  }} className="w-full bg-[#009688] text-white py-2 rounded">Chọn tự động</button>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg border border-gray-300 overflow-hidden">
              <div className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-500 text-sm">{(selectedSlots[`date-${selectedDate}`] || []).length > 0 ? `Đã chọn ${(selectedSlots[`date-${selectedDate}`] || []).length} khung giờ` : "Chưa chọn thời gian"}</p>
                </div>
                <button onClick={() => toggleDay(`date-${selectedDate}`)} className="flex items-center gap-2 hover:opacity-80" style={{ color: "#009688" }}>
                  <Edit size={18} />
                  <span>Chỉnh sửa</span>
                </button>
              </div>
              ...
              {expandedDay === `date-${selectedDate}` && (
                <div className="border-t border-gray-200 p-4 bg-gray-50">
                  <div className="mb-4 flex items-center justify-between">
                    <p className="text-sm"><span className="font-medium">Đã chọn: </span><span style={{ color: "#009688" }}>{(tempSelectedSlots[`date-${selectedDate}`] || []).length}</span></p>

                    {/* NEW: Xin nghỉ checkbox (bấm sẽ mở dialog confirm với ô nhập lí do) */}
                    <div className="flex items-center gap-2">
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        <input type="checkbox" onChange={openDayOffDialog} className="form-checkbox h-4 w-4" />
                  ...
                  <div className="grid grid-cols-6 gap-2 mb-4">
                    {timeSlots.map((slot, idx) => (
                      <button key={idx} onClick={() => toggleSlot(`date-${selectedDate}`, idx)} className={`py-2 px-3 rounded text-sm border transition-colors ${ (tempSelectedSlots[`date-${selectedDate}`] || []).includes(idx) ? "text-white" : "bg-white text-gray-700 border-gray-300" }`} style={ (tempSelectedSlots[`date-${selectedDate}`] || []).includes(idx) ? { backgroundColor: "#009688", borderColor: "#00796B" } : {} }>
                        {slot}
                      </button>
                    ))}
                  </div>

                  <button className="w-full text-white font-medium py-3 rounded-lg transition-opacity hover:opacity-90" style={{ backgroundColor: "#009688" }} onClick={() => {
                    // Xử lý lưu selectedSlots cho date cụ thể (nếu cần)
                    setSelectedSlots((prev) => ({
                      ...prev,
                      [`date-${selectedDate}`]: tempSelectedSlots[`date-${selectedDate}`] || [],
                    }));
                    setExpandedDay(null);
                    setTempSelectedSlots({});
      ...
      {showDayOffDialog && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-100 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 relative animate-[scale-in_0.2s_ease-out]">
            <button onClick={cancelDayOff} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition-colors"><X size={24} /></button>
            ...
            <h3 className="text-2xl font-bold text-gray-800 text-center mb-2">Xác nhận xin nghỉ</h3>

            <div className="bg-gray-50 rounded-lg p-4 mb-4">
              <p className="text-gray-700 text-center mb-3">Bạn có chắc chắn muốn xin nghỉ vào:</p>
            ...
            <label className="text-sm font-medium text-gray-700 mb-2 block">Lý do (bắt buộc)</label>
            <textarea value={dayOffReason} onChange={(e) => setDayOffReason(e.target.value)} placeholder="Nhập lý do xin nghỉ..." className="w-full p-3 border border-gray-300 rounded-lg mb-4 min-h-[100px] focus:outline-none" />

            <div className="flex gap-3">
              <button onClick={cancelDayOff} disabled={dayOffLoading} className="flex-1 bg-gray-200 text-gray-700 font-semibold py-3 px-6 rounded-lg hover:bg-gray-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed">Không</button>
              <button onClick={confirmDayOff} disabled={dayOffLoading || !dayOffReason.trim()} className="flex-1 bg-[#009688] text-white font-semibold py-3 px-6 rounded-lg hover:bg-[#00796B] transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                {dayOffLoading ? (
                  <>
